...
class MonthlyConsumeAmount < ApplicationRecord
  ...
  def self.calculate_average_amount_by_month
    ...
    total_amount = self.where('month <= ?', this_month).where('month >= ?', last_year).sum(:amount)
    number_of_recorded_months = self.count('distinct month')
  ...
  def self.get_number_of_consumed_products(date)
    self.where(month: date).count('distinct product_id')
  end

  def self.calculate_monthly_consume_amounts(date)
    self.where(month: date).sum(:amount)
  end
end
